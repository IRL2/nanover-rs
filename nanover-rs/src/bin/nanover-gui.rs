...
impl log::Log for UILogger {
    ...
    fn log(&self, record: &log::Record) {
        if self.enabled(record.metadata()) && record.target().starts_with("nanover") {
...
impl Client {
    ...
    pub fn get_simulation_list_force(
        ...
        // We only communicate with the nanover server we implement here. We
...
struct MyEguiApp {
    ...
    frame_interval: NumericField<usize>,
    force_interval: NumericField<usize>,
