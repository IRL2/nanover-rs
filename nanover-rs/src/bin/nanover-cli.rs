...
use log::LevelFilter;
use log::{error, info};
use nanover_rs::application::{cancellation_channels, main_to_wrap, AppError, Cli};
use std::error::Error;
...
fn main() -> ExitCode {
    ...
    if std::env::var("RUST_LOG").is_ok() {
        ...
        builder
            .filter_module("nanover_rs", verbosity_level)
            .filter_module("nanover_cli", verbosity_level)
            .format_target(false)
            .init();
    }
